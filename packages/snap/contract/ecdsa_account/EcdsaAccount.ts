...
import EcdsaAccountContractArtifactJson from './target/EcdsaAccount.json' assert { type: 'json' };
...
export class EcdsaAccountContract extends ContractBase {
  ...
  public static deploy(wallet: Wallet, signing_pub_key_x: (bigint | number)[], signing_pub_key_y: (bigint | number)[]) {
    return new DeployMethod<EcdsaAccountContract>(Point.ZERO, wallet, EcdsaAccountContractArtifact, Array.from(arguments).slice(1));
  }
  ...
  public static deployWithPublicKey(publicKey: PublicKey, wallet: Wallet, signing_pub_key_x: (bigint | number)[], signing_pub_key_y: (bigint | number)[]) {
    return new DeployMethod<EcdsaAccountContract>(publicKey, wallet, EcdsaAccountContractArtifact, Array.from(arguments).slice(2));
  }
  ...
  public methods!: {
    ...
    /** signing_public_key() */
    signing_public_key: (() => ContractFunctionInteraction) & Pick<ContractMethod, 'selector'>;
  };
}
